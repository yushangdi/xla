...
class PallasTest(unittest.TestCase):
  ...
  # @unittest.skipIf(xr.device_type() != 'TPU', "This test only works on TPU.")
  # # This test failed with `Failed to parse the Mosaic module`
  ...
  def test_flash_attention_wrapper_with_dynamo(self):
    ...
    expected_o = self._attention(q, k, v)
    print(o_no_causal.cpu() - expected_o.cpu())
    ...
    print(o_with_causal.cpu() - expected_o.cpu())
    self.assertFalse(
  ...
  @unittest.skipIf(xr.device_type() != 'TPU' or tpu.version() < 3,
                   "This test only works on TPUv3+.")
  def test_flash_attention_impl(self):
  ...
  @unittest.skipIf(xr.device_type() != 'TPU' or tpu.version() < 3,
                   "This test only works on TPUv3+.")
  def test_flash_attention_bwd_dkv(self):
  ...
  def test_flash_attention_backward(self):
    ...
    # shape = (2, 1, 128, 2) # 4, 2, 128, 8
    shape = (4, 2, 128, 8) # 4, 2, 128, 8
    
    q = torch.randn(shape, requires_grad=True).to("xla")
    k = torch.randn(shape, requires_grad=True).to("xla")
    v = torch.randn(shape, requires_grad=True).to("xla")
    q.retain_grad()
    k.retain_grad()
    v.retain_grad()
    ...
    torch.manual_seed(42)
    q2 = torch.randn(shape, requires_grad=True).to("xla")
    k2 = torch.randn(shape, requires_grad=True).to("xla")
    v2 = torch.randn(shape, requires_grad=True).to("xla")
    q2.retain_grad()
    k2.retain_grad()
    v2.retain_grad()
    o2 = self._attention(q2, k2, v2)
    loss2 = o2.sum()
    loss2.backward()
    q2_grad = q2.grad
    k2_grad = k2.grad
    ...
    # print(v2_grad.cpu())
    for i in [(q2_grad, q_grad), (k2_grad, k_grad), (v2_grad, v_grad)]:
    # for i in [(v2_grad, v_grad)]:
      print(i[0].cpu())
      print(i[1].cpu())
      print(i[0].cpu() - i[1].cpu())
      self.assertTrue(torch.allclose(i[0].cpu(), i[1].cpu(), atol=1e-05))
    jax.config.update("jax_default_matmul_precision", "default")
