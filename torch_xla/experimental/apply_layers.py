...
def apply_layers(layers: Iterable[torch.nn.Module],
                 input_data,
  ...
  `functorch.compile.min_cut_rematerialization_partition` to use min-cut based activation
  checkpointing. You may also write your own partitioner to insert any custom logic such
  as host offloading of activations.
  """
  ...
  def one_layer(carry, params_buffers):
    ...
    # `torch.zeros(1)`, for additional clarity. There is no extra
    # computation since we discard `ys` right after.
    return output, torch.zeros(1, device=torch_xla.device(), requires_grad=True)
