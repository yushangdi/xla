...
def scan(fn: Callable[[Carry, X], tuple[Carry, Y]],
         init: Carry,
         xs: X,
         partition_fn=default_partition) -> tuple[Carry, Y]:
  ...
  Args:
    ...
    partition_fn: since `scan` uses AOTAutograd to trace `fn`, you may override what computation
                  happen in the forward and backward passes by specifying different partition
                  functions. `default_partition` implies no activation checkpointing. You may
                  specify `functorch.compile.min_cut_rematerialization_partition` to use min-cut
                  based activation checkpointing. You may also write your own partitioner to
                  insert any custom logic such as host offloading of activations.
...
def value_and_grad_partitioned(
  ...
  """
  Given a user `fn` to be scanned over the major axis of the input `xs` PyTree
  and an initial carry object `init`, symbolically traces `fn` and returns two
  functions, `forward` and `backward`, where `forward` packs intermediate
  activations in the output `y`, and `backward` reads intermediate activations
  from the input `x`. `backward` also reads `grad_y` and outputs `grad_x` via
  its output `y`. Finally, `backward` takes in `grad_carry` from its input `carry`
  and returns new tangents through its output `carry`. More formally:

    forward(carry, x) -> (new_carry, (y, activations))
    backward(grad_new_carry, (grad_y, activations)) -> (grad_carry, grad_x)

  where `grad_y` is the gradient w.r.t `y` from `forward`, and `grad_new_carry`
  is the gradient w.r.t. `new_carry` from the `forward`.
  ...
  with torch.enable_grad():
    ...
    _, bwd_out_spec = tree_flatten((fake_carry_pytree, fake_x_pytree))
    out = fn_compiled(fake_carry_pytree, fake_x_pytree)
    out, fwd_out_spec = tree_flatten(out)
  ...
  def forward(carry, x):
    ...
    carry, y = tree_unflatten(actual_out, fwd_out_spec)
    y = (y, activations)
    return carry, y
  ...
  def backward(carry, x):
    ...
    grad_y, _ = tree_flatten(grad_y)
    out = bwd_graph(*activations, *grad_new_carry, *grad_y)
    grad_carry, grad_x = tree_unflatten(out, bwd_out_spec)
...
class Scan(torch.autograd.Function):
  ...
  def backward(ctx, grad_carry, grad_ys):
    ...
    # 2 comes from `pytreeify`. Other 2 are the `forward`, `backward` arguments
    # passed to the `forward` method.
    gradient_skipper = _GradientSkipper(ctx, starting_index=4)
    grad_init = gradient_skipper.process(grad_init)
    grad_xs = gradient_skipper.process(grad_xs)
    return None, None, grad_init, grad_xs


class _GradientSkipper:
  """
  Helper object to replace zero gradients with `None` gradients, if
  the `needs_input_grad` value for the corresponding input tensor is False.
  """

  def __init__(self, ctx, starting_index: int) -> None:
    self.ctx = ctx
    self.param_idx = starting_index

  def process(self, pytree):
    flat, spec = tree_flatten(pytree)
    flat = [
        v if self.ctx.needs_input_grad[self.param_idx + i] else None
        for i, v in enumerate(flat)
    ]
    self.param_idx = self.param_idx + len(flat)
